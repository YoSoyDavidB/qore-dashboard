...
export function ChatPanel({ onClose }: ChatPanelProps) {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "HERMES online. \u00bfEn qu\u00e9 trabajamos hoy?", timestamp: new Date() },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      const reply = data.response ?? data.message ?? data.error ?? "Sin respuesta";
      setMessages((m) => [...m, { role: "assistant", content: reply, timestamp: new Date() }]);
    } catch {
      setMessages((m) => [
        ...m,
        { role: "assistant", content: "Error de conexi\u00f3n con HERMES.", timestamp: new Date() },
      ]);
    }
    setLoading(false);
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-[500px] flex flex-col rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl z-50 overflow-hidden">
      {/* Header */}
